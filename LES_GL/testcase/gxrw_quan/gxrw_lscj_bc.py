...
def test_login02(browser):
    ...
    with allure.step('点击螺栓抽检页签'):
        wk.locator(*allPages.app_gxrw_lscj).click()
        # 关键等待：给弹窗加载留足够时间（可根据实际情况调整秒数）

    with allure.step("填写实际施工值"):
        wk.locator(*allPages.app_gxrw_lscj_sjsgz).send_keys(20)
    ...
    #     wk.locator(*allPages.app_gxrw_lscj_tj).click()
    #点击保存按钮保存任务
    with allure.step('点击保存任务'):
        wk.locator(*allPages.app_gxrw_lscj_bc).click()
        time.sleep(3)
    #点击法兰挂牌页签
