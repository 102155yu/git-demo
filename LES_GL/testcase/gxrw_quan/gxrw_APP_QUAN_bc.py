...
def test_login02(browser):
    ...
    with allure.step('点击页签法兰拆卸'):
        wk.locator(*allPages.app_gxrw_flcx).click()
        # 关键等待：给弹窗加载留足够时间（可根据实际情况调整秒数）
        # 点击法兰拆卸工具数量
    with allure.step('点击法兰拆卸工具数量'):
        wk.locator(*allPages.app_gxrw_flcx_cxgjsl).send_keys('1')
        # 点击法兰拆卸拉伸力
    with allure.step('点击法兰拆卸拉伸力'):
        wk.locator(*allPages.app_gxrw_flcx_cxlsl).send_keys('1000')
        # 点击拆卸后照片
    with allure.step('点击拆卸后照片'):
        wk.locator(*allPages.app_gxrw_flcx_cxhzp).click()
        time.sleep(2)
        # 输入图片路径并按回车确认（模拟手动输入文件地址）
    with allure.step('输入图片路径并确认上传'):
        # 输入图片绝对路径，r前缀避免反斜杠转义
    ...
    with allure.step('点击填写施工人员'):
        wk.locator(*allPages.app_gxrw_flcx_sgry).send_keys('余家傲')
    ...
    with allure.step('点击紧固照片'):
        wk.locator(*allPages.app_gxrw_yzqr_jgzp).click()
        time.sleep(2)
        # 输入图片路径并按回车确认（模拟手动输入文件地址）
    with allure.step('输入图片路径并确认上传'):
            # 输入图片绝对路径，r前缀避免反斜杠转义
