...
def test_login02(browser):
    ...
    # 进入登录
    with allure.step('进入LES登录页，登录设备员账号'):
        login = LoginPage(browser)
        login.login(LOGIN_URL_PC, USERNAME_sby_80, PASSWD)
    time.sleep(1)

    with allure.step("设备员新增计划"):
        LES.execute_full_sby_Plan_flow()

    with allure.step('退出登录'):
        LES.execute_full_log_out_flow()

    with allure.step("登录专工账号"):
        login = LoginPage(browser)
        login.input_credential_and_login(USERNAME_zg_YJA, PASSWD)

    with allure.step("专工审核计划 发布计划"):
        LES.execute_full_zg_shjh_flow()
